...
def _delete_profile(ssid: str, interface: str) -> None:
    """刪除指定 SSID 的 WiFi profile（best-effort，失敗僅記錄警告）。"""
    rc, out, err = _run(
    ...
    if rc != 0:
        print(f"[WARN] 刪除 profile 失敗: {err.strip() or out.strip()}", file=sys.stderr)
    else:
        print(f"[INFO] 已刪除失敗的 profile: {ssid!r}")
...
def main() -> int:
    ...
    if _wait_for_connection(args.ssid, interface, args.timeout):
        ...
        if profile_added:
            _delete_profile(args.ssid, interface)
        return 1
